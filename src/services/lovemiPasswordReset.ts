...
export function enqueueLovemiPasswordReset(ids: string[]) {
  const { accounts } = useEmailStore.getState()
  const byId = new Map(accounts.map((a) => [a.id, a]))
  for (const id of ids) {
    const a = byId.get(id)
    if (!a || isDemo(a.email, a.id)) continue
    if (!(a.refreshToken && a.clientId)) continue
    if (pendingIds.includes(id)) continue
    pendingIds.push(id)
  }
  void drainResetQueue()
}

/** 缺站内密码的已注册账号：排队重置 */
export function enqueueResetMissingPasswords() {
  const ids = useEmailStore
    ...
    .map((a) => a.id)
  if (!ids.length) {
    useEmailStore.getState().setToast('没有需要重置站内密码的账号')
    return
  }
  enqueueLovemiPasswordReset(ids)
}

async function drainResetQueue() {
  if (draining) return
  draining = true
  const { setToast, applyLovemiPasswordResets, probing, registering } = useEmailStore.getState()
  if (probing || registering) {
    draining = false
    return
  }
  if (!window.lovemi?.resetLovemiPassword) {
  ...
  let ok = 0
  let fail = 0
  if (pendingIds.length) setToast(`重置站内密码队列：${pendingIds.length} 个（串行）`)
  ...
  while (pendingIds.length > 0) {
    ...
    if (!account?.refreshToken || !account.clientId) continue

    try {
      let result = await window.lovemi.resetLovemiPassword({
        email: account.email,
      ...
      if (!result.ok && /too many|rate|429/i.test(result.error || '')) {
        ...
        result = await window.lovemi.resetLovemiPassword({
          email: account.email,
          refreshToken: account.refreshToken,
          clientId: account.clientId,
          proxyUrl: outbound.proxyUrl,
  ...
  draining = false
  if (ok + fail > 0) setToast(`站内密码重置完成：成功 ${ok} · 失败 ${fail}`)
